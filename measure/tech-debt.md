...
| 2026-05-08 | mobile_responsive_ui | Mobile-responsive benchmark UI implementation | Low | Resolved | LeaderboardTable cards on mobile, run detail grid responsive, GameCanvas scales with CSS, safe-area padding, DESIGN.md updated with responsive guidelines |
| 2026-05-08 | visual_regression_detection | Phase 1-4: Visual diff engine, diff-config, RegressionAlert, ComparisonView diff toggle | Low | Resolved | lib/visual-diff.ts (11 tests), lib/diff-config.ts (6 tests), lib/diff-storage.ts (4 tests), RegressionAlert.tsx (5 tests), ComparisonView diff mode (8 tests) |
| 2026-05-08 | benchmark_alert_system | Phase 1-5: AlertRulesEngine, alert-store, alert-transport, AlertHistory component | Low | Resolved | 43 tests passing across 5 files (alert-rules, alert-store, alert-transport, alert-integration, AlertHistory.test.tsx) |
| 2026-05-09 | multi_resolution_support | Phase 1-5: Resolution field (32x32/64x64/128x128), validate-run-logic with resolution specs, ResolutionBadge component, leaderboard resolution filtering | Low | Resolved | 527 tests pass; schema updated with optional resolution enum |
| 2026-05-09 | ml_quality_scoring | Phase 1: Feature extraction (color histogram, edge density, transparency, grid alignment), scorer module, AutomatedScoreBadge | Low | Resolved | 28 tests pass; build TypeScript check emits pre-existing WASM error on linux/x64 |