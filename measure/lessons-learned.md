...
- (2026-05-02, typescript_strict_mode) TypeScript type-only imports (import type { X }) must be explicitly included - `as unknown as Type` in tests requires the Type to be imported; test file was missing GameEngineTestRunner import causing TS2304 at compile level
- (2026-05-02, leaderboard_system) Array.sort() with localeCompare for case-insensitive string comparison: `toLowerCase().localeCompare()` - localeCompare returns 1 for 'G' > 's', but descending sort needs `-comparison` which flips the non-symmetric comparison; test expectations must account for case-sensitive ASCII ordering within same case
