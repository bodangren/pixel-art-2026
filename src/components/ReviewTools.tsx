...
const ReviewTools: React.FC<ReviewToolsProps> = ({
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-6 p-4 bg-slate-800/50 border border-slate-700 rounded-lg ${className}`}
      role="toolbar"
    ...
    >
      <div className="flex flex-col gap-1 min-w-[150px]">
        <label htmlFor={`${idPrefix}-zoom`} className="text-xs font-bold uppercase tracking-wider text-slate-400">
          Zoom: <span aria-live="polite">{Math.round(zoom * 100)}%</span>
        </label>
        <input
          id={`${idPrefix}-zoom`}
          type="range"
